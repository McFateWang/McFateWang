...
def mkdir(path):
    ...
    isExists = os.path.exists(os.path.join("C:\mytools\my_py_pic", path))
    # 如果不存在
    if not isExists:
        print('makedir', path)
        # 创建文件夹
        os.makedirs(os.path.join("C:\mytools\my_py_pic", path))
        # 切换到创建的文件夹
        os.chdir(os.path.join("C:\mytools\my_py_pic", path))
        return True
    # 如果存在了就返回False
    else:
